    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.clubs.update", [$club->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.club.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $club->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="slug" id="slug" value="{{ old('slug', $club->slug) }}" required>
                            @if($errors->has('slug'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('slug') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id" required>
                                @foreach($categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $club->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
