    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.enrollments.update", [$enrollment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $enrollment->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('championship') ? 'is-invalid' : '' }}" name="championship_id" id="championship_id" required>
                    @foreach($championships as $id => $entry)
                        <option value="{{ $id }}" {{ (old('championship_id') ? old('championship_id') : $enrollment->championship->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('club') ? 'is-invalid' : '' }}" name="club_id" id="club_id" required>
                    @foreach($clubs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('club_id') ? old('club_id') : $enrollment->club->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('club'))
                    <div class="invalid-feedback">
